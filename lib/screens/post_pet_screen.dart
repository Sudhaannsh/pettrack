...
class _PostPetScreenState extends State<PostPetScreen> {
  ...
  // ... (keep all the existing location and image methods unchanged)
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        // Upload image first
        String imageUrl = await _lostFoundPetService.uploadImage(_imageFile!);

        // Create pet data map for lost/found pets collection
        final petData = {
          'name': _nameController.text.trim(),
          'breed': _breedController.text.trim(),
          'color': _primaryColor,
        ...
        await _lostFoundPetService.addLostFoundPet(petData);

        if (!mounted) return;
        Navigator.of(context).pop(true);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${_status == 'lost' ? 'Lost' : 'Found'} pet reported successfully'),
            backgroundColor: _status == 'lost' ? Colors.red : Colors.green,
          ),
        );
      } catch (e) {
  ...
  // ... (keep the rest of the build method unchanged)
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
