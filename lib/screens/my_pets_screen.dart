...
//         final pets = await _petService.getPetsByUserId(userId);
//         print(
//             'Loaded ${pets.length} pets from owned_pets collection'); // Debug log

//         // Print each pet for debugging
//         for (var pet in pets) {
//           print('Pet: ${pet.name}, Owner: ${pet.ownerId}, Lost: ${pet.isLost}');
//         }
...
// // Replace your existing _markAsFound method with this:
//   Future<void> _markAsFound(PetModel pet) async {
//     try {
//       // Get the lost report ID first
//       final lostReportId = await _petService.getLostReportId(pet.id!);

//       // Mark as found and remove report
//       await _petService.markPetAsFoundAndRemoveReport(pet.id!, lostReportId);
...
class _MyPetsScreenState extends State<MyPetsScreen> {
  ...
  Future<void> _loadPets() async {
    ...
    try {
      final userId = _authService.currentUser?.uid;
      print('=== DEBUG: Loading pets ===');
      print('Current user ID: $userId');

      if (userId != null) {
        final pets = await _petService.getPetsByUserId(userId);
        print('Loaded ${pets.length} pets from owned_pets collection');

        // Print each pet for debugging
        for (var pet in pets) {
          print(
              'Pet: ${pet.name}, Owner: ${pet.ownerId}, Lost: ${pet.isLost}, ID: ${pet.id}');
        }
  ...
  Future<void> _markAsFound(PetModel pet) async {
    try {
      // Get the lost report ID first
      final lostReportId = await _petService.getLostReportId(pet.id!);
      print('Lost report ID for ${pet.name}: $lostReportId'); // Debug log
      ...
      print('Error marking pet as found: $e'); // Debug log
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error updating pet: $e')),
        );
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              try {
                ...
                print('Total documents in owned_pets: ${snapshot.docs.length}');
